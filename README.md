...
**Security-hardened, de-clouded [Asuswrt-Merlin](https://github.com/RMerl/asuswrt-merlin.ng) for the ASUS RT-BE96U** (WiFi 7 / Broadcom BCM4916), firmware line **3006.102.x** — branded `reaper`. Current: **v1.6.0**.
...
3. **[docs/CHANGELOG.md](docs/CHANGELOG.md)** — big changes per version, v1.0 → v1.6.0.
...
- Current version: **v1.6.0** (everything through v1.4.8 is validated on the physical RT-BE96U, including the USB third-factor flow; v1.5.0a's AI Advisor + network-diagnostics tier are validated on the physical RT-BE96U; v1.5.0b [Traffic Analyzer 200ms Live mode + diag-aware Advisor], v1.5.0c [SPDX headers + font OFL], and v1.5.0d [de-ASUS UI rebrand: REAPER1 banner, RLogo AiMesh, live rail clock] are build-verified; **v1.6.0** [hardening pass + full 24-language UI] is built, shipped, and flashed to the RT-BE96U with its checklist owed). Per-version history: [docs/CHANGELOG.md](docs/CHANGELOG.md).
