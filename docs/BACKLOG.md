...
1. **[P1] 2026-09-17 apply-window fixes — BUILT in the canon tree, uncommitted; metal owed.**
   One `restart_qos;restart_firewall` (a QoS page Apply with L4S switched on) on the owner's RT-BE96U
   rebuilt the firewall; the three Reaper layer applies, hook.sh, rwatch 3c and the out-of-tree
   `reaper_dnsi` carve-out watcher then fought over FORWARD position 1 for fifty seconds and the
   restricted devices lost DNS four times. Six fixes: Gatekeeper RETURNs router-redirected DNS
   (`--ctstate DNAT --ctorigdstport 53`) ahead of its per-network DROP; hook.sh computes its pin
   position with rwatch 3d's classifier (`_front_pos`) instead of `-I 1`; Warden's apply creates and
   replays the sets before the teardown and replays the cache in ONE awk pass (was 80 passes = 17 s
   measured); rwatch 3c confirms on two ticks and heals through `heal.sh`, which re-checks under the
   lock; the rules engine puts an intercept aimed at the DNS Health Check target in `REAPER_FWHC`
   and `rdnshc` closes/opens it (`hcgate.sh`) on FAILOVER/RESTORED; the L4S toggle and the WMM
   downstream stamp are REMOVED (L4S never accepted by the TM on this port and its rejection used to
   abandon the priority correction; WMM halved Wi-Fi throughput on metal). Host evidence: `make mk-rc` exit 0, static 13/13,
   walker + Warden suites green, hook.sh generated by the real generator and driven through six
   tables, the new rwatch/hwqos/hcgate/replay blocks extracted from the C strings and driven under
   dash and BusyBox ash (52 checks). Test image RT-BE96U MCP v3.1.8_BETA_r4 (sha 1d86c8a1) shipped
   2026-09-17 09:00, verify 34/34, and flashed. **Metal, in this order:** (a) flash; QoS Apply →
   no dualq line in syslog, `tmctl getqcfg` shows priorities 5..1, the page has no L4S or WMM row;
   (b) `service restart_firewall` → no `reaper_dnsi` lines, `iptables -S FORWARD` keeps REAPER_DNSV
   at 1 and REAPER_HOOK_F at 2 while the /jffs scripts are still there, no rwatch Warden line at the
   next tick; (c) NAT tab: Service Intercept udp+tcp 53 → 10.20.0.98 from br0, Exempt = the
   resolver object, DNS Health Check watching 10.20.0.98; then delete
   `/jffs/scripts/{firewall-start,nat-start,services-start,reaper_dnsi_watch.sh}`, kill the watcher,
   `service restart_firewall`, confirm `iptables -t nat -S REAPER_FWHC` holds the two DNATs and an
   internet-only device still resolves (the GKF `ctorigdstport 53` counters climb); (d) stop AdGuard →
   "intercept gate CLOSED" and clients resolve through the router; start it → "intercept gate OPEN".
