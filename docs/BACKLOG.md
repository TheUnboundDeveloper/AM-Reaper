...
- **[P2] DHCP-hostname stored XSS — the networkmap path is still open.** A device's
  self-announced DHCP hostname reaches admin views unescaped and lands in an HTML sink,
  so any device on the LAN can plant script that runs with the administrator's session
  without ever logging in (audit finding **H1**, `code-audit-2026-07-31-inherited`;
  origin `web.c:10561` raw → `fromNetworkmapd` → three admin views). v2.3.5 closed the
  **DHCP Leases page's** instance of this at the emit point, but only that page — the
  networkmap → admin-view path is untouched. Fixing it is one `htmlEncode` at the
  source, but it needs a sweep of the three consuming views first so nothing
  double-encodes. [owed]
