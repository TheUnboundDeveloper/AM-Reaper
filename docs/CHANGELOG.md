...
## Unreleased — Warden's outbound direction actually blocks, the firewall pages explain themselves, and two field reports about numbers that did not add up
*Source only. This work sits on top of the built v2.4.1 head and **has not been compiled, flashed or run on hardware** — no version number has been assigned to it yet. Everything below is verified by construction (generated scripts parsed, emitters compiled against stubs, ipset commands run against a real ipset, page helpers executed in a browser engine) and by nothing else.*
