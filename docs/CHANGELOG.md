...
- **Fixed (worked around): a router can reach a state where saving a setting never completes, while everything else keeps working.** The symptom is unhelpfully quiet — the router stays up, SSH and the web interface still respond, traffic still flows — but one process is stuck forever, a CPU core is being consumed doing nothing, logs quietly fill, and whatever setting was being written is never actually saved. It gets more likely the longer a router has been up.
  - **The cause is in a closed Broadcom library** (`libnvram.so`), and it was found by catching it live and then confirming it instruction-by-instruction in the shipped binary. Every process that touches settings opens a small kernel channel identified by a number, and this library uses **the process's own ID as that number**. If a second process is already using it — which happens naturally, because process IDs get reused as a router runs — the library tries the next nine numbers along. The bug is that the number it is trying and the record of "which process am I" are **the same piece of memory**. So the moment it has to try a different number, it corrupts its own record of its identity. The next call notices the mismatch, wrongly concludes the process has changed, and starts over — **without closing the channel it already opened**. Each round leaks one, and drifts further, until all ten numbers are used up. After that the library retries a failing send *forever*, with no pause, while holding the lock every other settings operation needs — so that process is finished, and it spends the rest of its life issuing about sixteen system calls per attempt as fast as the processor allows.
  - **Why it compounds:** the leaked channels stay held for as long as the stuck process lives, and it never exits — so those numbers stay occupied and make the *next* collision more likely. On a busy router this quietly gets worse with uptime.
  - **The workaround.** Every step of that chain depends on the very first attempt failing. If it succeeds, nothing drifts, nothing leaks, nothing spins. Reaper now ships a small piece of its own code that sits beside the Broadcom library (it does **not** modify it) and, *only* when that first attempt has already failed with "address in use", asks the kernel to pick a free number instead — which is exactly what Broadcom's own second channel already does. Everything else is passed through untouched.
  - **It is switched off by default.** The mechanism it uses applies to every program on the router, and this has not yet been run on real hardware, so it ships inert and changes nothing until you deliberately enable it with `nvram set wlcsm_bindfix=1`, then commit and reboot. Setting it back to `0` removes it on the next boot. When it is on, every time it rescues a collision it writes a line to the system log naming the process and number involved, so you can see whether the problem exists on your router at all.
