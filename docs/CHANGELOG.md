...
*In the source tree and **built on RT-BE96U in both variants as a test image** — the rung is **not cut**: there is no patch series, no provenance stamp, and the four sibling models have not taken it yet. Nothing is published. Five changes: two field reports, two owner requests, and one regression from v2.4.6.*
