...
- **Fixed: the built-in speed test could abort partway through, and got more likely to the longer a session went on.** The test allows itself a few retries when the network hiccups. That allowance was reset when a run succeeded, and when the allowance ran out — but *not* when a run failed with none left. So the shortfall carried forward, and a later test could start with no retries at all and give up on the first small glitch. The allowance is now reset for every run.
