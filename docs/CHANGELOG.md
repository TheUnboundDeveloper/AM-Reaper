...
*Built on RT-BE96U (MCP), 20/20 on the staged-image gate. **Not yet run on hardware** — everything below is verified by construction (generated scripts parsed, emitters compiled against stubs, ipset commands run against a real ipset, page helpers executed in a browser engine) and by a clean build, and by nothing else.*
