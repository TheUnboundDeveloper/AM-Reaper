...
dispatchable on its own — `model: RT-BE92U`, building to the `96765GW` profile — but is
deliberately kept out of the `all` standard-fleet fan-out, so it ships as experimental
prereleases; its platform tree ships in the pinned upstream base, so unlike GT-BE98 it needs
no platform archive) — which computes `git diff be96u-only <branch>`
