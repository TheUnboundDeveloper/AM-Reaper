...
**RT-BE92U is retired from the roster** (owner, 2026-09-10): upstream Merlin has
taken that model on, so a second build of it serves nobody. It is not in the
model list, not in the `all` fan-out and not passed to `refresh_manifest`, which
merges per model and therefore leaves its existing update lines frozen at
v3.1.2-beta rather than withdrawing them from anyone still running it. Its
replacement is to be the GT-BE19000.

The model is **dormant, not deleted** - the `rt-be92u` branch, its worktree,
`overlays/RT-BE92U.patch`, its provenance entries and the per-model tables in
`build-scripts` all stay, and `build_be92u.sh` still builds it under
`REAPER_BUILD_RETIRED=1`. The mapping below is kept for that reason.

It was the odd one out mechanically too: BCM6765, not BCM4916, so it builds to
the `96765GW` profile where the other five build `96813GW` (`ci/build_one.sh` and
`ci/container_build.sh` derive the profile, branch and u-boot symbol from
