...
`port_sibling_v2.sh` (maintainer tooling, outside this repo) — which computes `git diff be96u-only <branch>`
...
matrix at v3.1.4. That is not the same as being
...
| `build-scripts/ci/build_one.sh` | One model, one variant, no ship. Model→branch/target mapping, identity assertion, dict-integrity gate, pass-1 excerpt capture. |
