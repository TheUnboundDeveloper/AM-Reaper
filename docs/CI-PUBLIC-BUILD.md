...
canon. The other four models are separate branches, normally generated by
`build-scripts/port_sibling_v2.sh` (a newer sixth model, **RT-BE92U** / BCM6765, is
part of the `all` fleet fan-out (building to the `96765GW` profile) and also dispatchable on its
own, but it is **always published as a prerelease** (release.yml), so it never lands as a standard
release alongside the five BCM4916 models; its platform tree ships in the pinned upstream base, so
unlike GT-BE98 it needs no platform archive) — which computes `git diff be96u-only <branch>`
and therefore needs a clone holding all five branches. A clean-room checkout has
one. Three things had to be solved before CI could build a sibling; all three are
now in place.
...
Zero content mismatches. The banner art comes from [`reaper-mockups/`](../reaper-mockups/)
...
| `overlays/` | Four sibling identity overlays + the GT-BE98 platform archive with its SHA-256. |
