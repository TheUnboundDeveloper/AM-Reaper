...
| **M3** | `ASUSLogo.png` (ASUS trademark asset) added by patch `0144`, used as the AiMesh backdrop. | **FIXED (v1.5.0d, patch `0153`).** The AiMesh backdrop now uses a Reaper asset (`RLogo.png`) and `ASUSLogo.png` is removed from the tree. Same rebrand also swapped the header/login logo to `REAPER1.png`. Written credit to ASUS/Merlin (text) stays — legitimate nominative use. |
