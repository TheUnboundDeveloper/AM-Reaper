...
*Questions marked ⚠ VERIFY ON UNIT are the ones to confirm on a physical RT‑BE96U before publishing this guide: the rescue button (Reset vs WPS) and LED sequence, the exact recovery image the BE96U’s restoration flow expects, whether a second/rollback image exists, and any minimum‑version/anti‑rollback enforcement on downgrade.*
