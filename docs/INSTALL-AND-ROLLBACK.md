...
**Applies to:** Reaper (Asuswrt‑Merlin 3006.102.8 fork), current release **v2.3.2** (`3006.102.8_Reaper_v2.3.2`, the latest full-fleet build; **v2.3.3** is RT‑BE96U-only so far), for the **ASUS RT‑BEXXU** (Broadcom BCM4916, WiFi 7). This guide is written for the RT‑BEXXU specifically; Reaper also ships for the sibling BCM4916 models **RT‑BE86U**, **RT‑BE88U**, **GT‑BE98**, and **GT‑BE98 Pro**, and the **same flash and rollback procedure below applies to each — using that model's own image**.
