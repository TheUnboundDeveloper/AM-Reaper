...
A security-hardened, **de-clouded** fork of **Asuswrt-Merlin** for the **ASUS RT-BE Series** (WiFi 7 / Broadcom), firmware line **3006.102.x**. The **RT-BE96U** is the primary, hardware-validated model; the **RT-BE86U**, **RT-BE88U**, **GT-BE98**, **GT-BE98 Pro** and, from v3.1.4, the **GT-BE19000** (BCM4916) are built from per-model branches of the same tree. Current version **v3.1.0** <!--@pubver--> — the newest **published** release (2026-09-08 <!--@pubdate-->), on all five main models, both variants each. Newer rungs also appear on the Releases page as **pre-releases**, marked `_BETA` in the filename and on the router's dashboard; the router's own update check offers those only when its beta channel is switched on. The source tree is normally ahead of that: it currently sits at **v3.2.5** <!--@treever--> (RT-BE96U). "Current version" always means the newest published release — the newest image a user can actually install — because source rungs are cut more often than releases are published. See [`CHANGELOG.md`](CHANGELOG.md) and [`RELEASE-NOTES.md`](RELEASE-NOTES.md).
...
- **Branding:** `reaper`. The build version reads `<MODEL> 3006.102.8_Reaper_v<X>` (e.g. `RT-BE96U …_Reaper_v2.5.3`).

## Scope & hard rules

- **Six-model RT-BE fleet.** RT-BE96U is primary and hardware-validated; the five BCM4916 siblings — RT-BE86U, RT-BE88U, GT-BE98, GT-BE98 Pro and, from v3.1.4, GT-BE19000 — build from per-model branches of the same tree. Built from `release/src-rt-5.04behnd.4916` via `make <target>` (`rt-be96u` / `rt-be86u` / `rt-be88u` / `gt-be98` / `gt-be98_pro` / `gt-be19000`). Each model ships two variants (Standard / + AI Advisor).
