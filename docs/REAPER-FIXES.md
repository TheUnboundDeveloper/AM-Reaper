...
- **Custom version string:** the security hardening catalogued here landed in **v1.0** and is carried unchanged through the current **v1.4.1** release (later versions added features and de-cloud removals, not new base-firmware findings — see [`CHANGELOG.md`](CHANGELOG.md)). The v1.0 image details below are the record of when these fixes shipped.
