...
**Applies to:** Reaper firmware, line `3006.102.8_Reaper_v<X>`, for the ASUS RT-BE96U (primary, hardware-validated) and the sibling RT-BE86U, RT-BE88U, GT-BE98 and GT-BE98 Pro. This guide describes the feature set as of the v2.7.0 source tree. The newest *published* release may be behind that; where a feature is newer than the image you are running, the page simply will not be there yet. See [`CHANGELOG.md`](CHANGELOG.md) for what each version added and [`BACKLOG.md`](BACKLOG.md) for what is still pending confirmation.
...
Take both, and take the Reaper one again after any significant change to those four features. The Reaper file **contains your device addresses and your domain lists** — keep it with your other private backups.
...
- The **left rail** is identical on every page. At the top is a live 24-hour router-time clock; above the "General" heading is the **Language** selector (all 25 languages; the active language is shown selected and English can always be chosen). Menu entries are the stock sections plus Reaper's own (Devices, Gatekeeper, Warden, AI Advisor on the MCP build). Installed **addons** (amtm, Diversion, scMerlin and the like) appear as their own entries on both the dashboard and the shell; an external "Help & Support" link opens in a new tab. At the foot of the rail a small **scythe mark** opens the About page; it stays at the bottom of the window while a long menu scrolls beneath it.
...
**Note:** the per-page reference [`VPN-ROUTING-GUIDE.md`](VPN-ROUTING-GUIDE.md) describes the v2.5.x version (OpenVPN only, IPv4 only, no countdown). v2.6.7 changed all three; this section follows the changelog and the page text.
