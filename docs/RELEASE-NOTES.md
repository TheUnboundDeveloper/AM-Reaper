...
**Models:** ASUS **RT-BEXXU** (primary, hardware-validated), plus the **RT-BE86U**, **RT-BE88U**, **GT-BE98**, and **GT-BE98 Pro** siblings (per-model branches of the same tree). **v2.1.4 is built on the RT-BEXXU (both variants, passing the 17-check verify gate); the last full-fleet ship was v2.1.2, and the v2.1.3/v2.1.4 sibling fan-out (RT-BE86U / RT-BE88U / GT-BE98 / GT-BE98 Pro) is in progress.** WiFi 7, Broadcom BCM4916.
...
The rungs after v2.1.0 are RT-BEXXU-first (the v2.1.3/v2.1.4 sibling fan-out is in progress). Headlines — per-version detail is in [`CHANGELOG.md`](CHANGELOG.md):
