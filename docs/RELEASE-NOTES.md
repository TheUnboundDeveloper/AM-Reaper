...
**Release:** v2.4.3 — **the v2.4.2 work, audited before it went anywhere.** This release carries everything in the v2.4.2 entry below — which published no images — plus the remediation of a security audit run against it, so **nothing here is a fix to code any user has ever installed**. The audit covered the shell scripts the router generates and runs as root, the new lines spliced into the DNS service's configuration, the JSON the web interface parses, the new pages and the traffic daemon's output. It came back **clean** on the things that would have mattered most: no way found to inject a command into the generated scripts or a directive into the DNS configuration, no cross-site scripting in the new pages, no new listening service, and the cross-site request protections correct on every control that changes settings. It found **six defects**, all in code v2.4.2 introduced; all six are fixed. Three could have been felt: a **malformed mesh-client file could crash the web interface** (the wired/wireless check validated the file's shape at two of its three levels, and the web server runs with full privileges — latent, since it first requires the ability to write into the router's temporary directory); a **website-name block rule with many names could have stopped DNS and DHCP for the whole network** (the code reading the generated DNS line used a smaller buffer than the code writing it, so a long list was cut in half, leaving either a rule that silently never matched or a broken directive that makes the DNS service refuse to start — both ends fixed, the writer now logging any names it had to omit and the reader refusing a partial line outright); and **Warden's new block breakdown could read as zero with two browser tabs open** (the statistics script used one fixed working file and the web interface runs it every thirty seconds, so two sessions overwrote each other and banked zeros — exactly the confusion the breakdown was added to solve). The other three are smaller: an object limit applied in one place and not its match, so past that limit the router asked the DNS service to fill lists it had never created; the DNS configuration fragment was the one file the firewall engine wrote without setting permissions, so it landed world-writable; and two networks sharing one bridge (the guest IoT network sits on the main bridge) made the traffic graph label the main network with the guest network's name. Two hardening changes came with them, the notable one a silent-failure mode rather than a leak: the router-self filter exempts your DNS servers automatically and read those addresses straight from the resolver file, where an address written with a prefix — `0.0.0.0/0` — is *accepted* by the firewall as "allow everything" at the top of the chain, quietly disabling the entire self-filter while every status indicator still reported it on. **Deliberately not fixed here:** the router's temporary directory is writable by every account with no restriction on deleting another's files, which is what makes the first defect reachable at all — stock ASUS/Merlin behaviour affecting every part of the system including the closed-source components, recorded with two candidate approaches and owed its own release and its own hardware pass. **Validation status: built on RT-BE96U, both variants; the four siblings are ported and are for CI.** Per-version detail in [`CHANGELOG.md`](CHANGELOG.md).

Folded into the release above (published no images): v2.4.2 — **Warden's outbound direction actually blocks, the firewall pages explain themselves, and two field reports about numbers that did not add up.** **Warden's "outbound" and "both" directions were blocking nothing:** the whitelist was checked before the outbound rules, and since every outgoing connection has one of your own devices as its source, any whitelist entry covering that device — including the "whitelist your own network" entry this feature's own lockout guidance tells you to keep — skipped the destination checks entirely, while inbound blocking of the same addresses worked perfectly. Destination checks now run first, in a chain of their own, and gained the loopback/LAN/own-address exemptions the outbound side never had. **Warden can now also filter what the router itself sends** (opt-in, off by default): a compromised router is precisely where a threat feed earns its keep, and it cannot lock you out — the web interface and SSH stay reachable from your network — but it can stop the router reaching things it needs, so DNS resolvers and time servers are exempted automatically and every block here is logged whether or not general Warden logging is on. Be clear-eyed about the ceiling: malware with full control of the router can remove these rules, so this is a speed bump and mainly a **detection signal**. **Warden's "total blocked" beside an empty country table is explained rather than fixed:** the total counts every block — manual, feeds and countries — while the table only ever counted countries, and because the feed rules are checked first (both simply drop, so this only decides which counter moves) the feeds absorb most hits and the country counters barely move; the total now shows its breakdown so the numbers add up. **Every tab of the firewall page now explains itself** — what it is for, how it is best used, a worked example you could actually type in, and a **?** button linking into a new [firewall guide](FIREWALL-GUIDE.md) that records the traps only the code knew. **Website-name objects fill themselves from the router's own DNS**, which v2.4.1 deferred on the stated grounds that the DNS service lacks the capability — **that was wrong**, it has had it all along and the check had been run against a stock binary from a different model and architecture; the ten-minute timer stays as the floor, because this only sees lookups the router itself answers. Also: the Traffic Analyzer's per-network view labels each network with its **name and VLAN** alongside the internal bridge name (`Guests (VLAN 52, br54)`) instead of the bare `br54` nobody configures, and **Wi-Fi devices connected through an AiMesh node are no longer listed as Wired** — the router now consults the mesh's own client list, so they report as Wi-Fi on the band they are really using. **Validation status: built on RT-BE96U (MCP variant only), 20/20 on the staged-image gate, and run on hardware; the noMCP variant and the four siblings are unbuilt and are for CI.** The four sibling branches were ported and their overlays regenerated and gated, but a clean build and a passing staged verify prove build and staging correctness, not that those routers boot — that remains owed on the siblings. Per-item detail in [`CHANGELOG.md`](CHANGELOG.md).

Previous release: v2.4.1 — **a firewall of Reaper's own, UPnP that consoles can talk to again, and mesh nodes you can update from one page.** This release folds in the v2.3.8, v2.3.9 and v2.4.0 source rungs, none of which published images — the firewall spanned all three, and shipping a half-built rules engine was not worth doing. **UPnP stopped consoles connecting to games:** a PS5 could not reach Call of Duty at all, with IPv6 pinholes on or off, and two separate faults were doing it. v2.3.5 moved every build to describing itself as an IGD:2 gateway, and while *discovery* stays backward-compatible the **description** the client fetches immediately afterwards does not — a console reading it recognised none of the services, never got a control address, and could not create a mapping. The code already force-downgrades that description for Microsoft clients for exactly this reason; the console simply is not covered by the exception. It is now IGD:1 for everyone (`upnp_igd2_desc=1` restores IGD:2), and because the IGD:1 description **omits the IPv6 firewall service entirely**, enabling pinholes now implies IGD:2 — otherwise that switch produced a daemon that supported pinholes and a description that hid them. The router also now logs which description each client was served **and what that client called itself**, which is the fact that was missing when this took a build to diagnose rather than a log line. Separately, an **upstream defect carried in the 3006.102.8 base** added an unqualified rule that tests every packet crossing the router — outbound included — against every active UPnP mapping, so a mapping on some port rewrote a LAN device's own outgoing connection to a remote server on that port; reverted. **The native firewall is now real and switched on by choice** (off by default): rules are written about named objects — a device, a network, a service, a country reusing the list Warden already downloads — and cover inbound, forwarded and outbound traffic on IPv4 and IPv6, with schedules, rate limits and per-rule logging, **adding** policy on top of the stock firewall rather than replacing it. **You cannot lock yourself out:** every apply starts a countdown and the router restores the previous ruleset by itself unless you confirm, driven by a scheduled task running a pre-written script rather than by the web server, so it still fires if what you just broke is the web interface. A protective set is always applied first, including the IPv6 neighbour-discovery traffic a network stops working without — a rule that killed it would otherwise pass the countdown unnoticed, because the operator's IPv4 session stays up throughout. Per-device internet defaults, source- and schedule-restricted port forwards, a compile preview generated by the same code that would apply it, and text backup/restore round it out. The firewall **Status** page stopped printing raw netfilter chains — 40-odd chains and several hundred counter rows, refetched every ten seconds, unreadable without prior `iptables` knowledge — and now reports posture in five groups, including the one most people need: **what of yours is reachable from the internet right now**. The engine's on/off switch reports a **measured** signal (the engine marks itself active as its last act, and only when its rules really went on) rather than echoing back the flag that was just written. Also: **mesh nodes** appear correctly and can be updated from the main router — the page had been filtering itself out of its own node list using a field the router never sends, so **the router listed itself as one of its own nodes**; the **setup wizard** no longer drops you onto an unthemed stock Wi-Fi page; the dashboard regains the internet on/off switch and USB disk selector the retired Network Map carried; the **Devices** page exports the client inventory as CSV, JSON or HTML; every **Security Posture** row on the dashboard now links to the page that owns the setting; and during a firmware flash the page behind the header no longer shows through it — the header was being dimmed with transparency, which does not fade an element so much as make it see-through. **Known and not fixed:** the Professional Wi-Fi page still takes ~15 s on its first load after an upgrade (the mechanism is understood; the fix depends on whether the cost is the page or the driver settling, which needs one measurement on hardware), and two firewall items are deliberately deferred — a rule tracer, and DNS-driven refresh of website-name objects, the latter because this router's dnsmasq is compiled without ipset support and rebuilding the most critical service on the box deserves its own rung *(**corrected 2026-08-14:** that reason was wrong — dnsmasq has had ipset support all along, the check having been run against a stock binary of a different model and architecture; the feature is implemented in the in-development rung above and needed no rebuild)*. **Validation status: built on RT-BE96U (both variants) and run on hardware; the four-model sibling fan-out is owed, and additionally needs the per-model overlays regenerated, since this rung touches two files those overlays carry.** Per-version detail in [`CHANGELOG.md`](CHANGELOG.md).

Release before that: v2.3.7 — **the Wi-Fi pages stop misleading you, mesh nodes show their firmware, and Professional opens quickly.** Five changes, all in the web interface and the small server behind it; nothing touches how traffic is forwarded. **The Professional Wi-Fi page** was asking the router for its wireless settings one at a time — 116 separate questions, none of the page drawable until the last answer arrived — and on a three-radio router 29 of those asked about a fourth radio that does not exist. It now asks once, about the radios the router actually has, and adapts by itself to any model. **Auto Scan now assigns the channel it told you was best:** on 5 GHz it used to rank individual narrow channels and then quietly apply the wider block *containing* the winner, so it could report channel 44 as best and set 36 — which read as the tool ignoring its own results. It now measures at the width the radio will really run, so the entry it ranks and the channel it sets are the same thing. It also takes the channels to sweep from what each radio itself reports rather than a fixed list, so a router with two 5 GHz radios sweeps each across only the channels that radio supports and other regions work without a code change, and when a sweep finishes it applies its winner instead of restoring the old channel and waiting for a button. **Two consequences worth knowing:** radar-detection (DFS) channels are deliberately not swept, because each needs a 60-second listening period with the radio off the air — up to ten minutes on some — which would turn a scan into a long outage; and applying a channel takes that radio off automatic channel selection until you set its control channel back to Auto. Both are now stated on the page. **The firmware page lists your AiMesh nodes** with the firmware each reports and whether it is online, and gives each online node a button that opens that node's own firmware page, where you choose an image and the node flashes itself — the image is never relayed through the main router. **A failed "Search for node" now records why:** there are two points where a discovered node is discarded for incomplete information and neither said anything, so the search came back empty with no explanation; both now write a line to the system log naming the node and exactly what was missing. That changes no behaviour — it makes an existing silence audible. Finally, a **Smart Connect** note claimed 6 GHz is left out by default "on this model", which is only true on some models while the page is shown on all of them; it has been removed. **Validation status: built and published on all five models** (2026-08-13, three assets each) through the public clean-room CI pipeline; on-hardware validation of this specific rung is not separately recorded. *(This line previously read "source only — not built on any model"; it was written at cut time and never updated once the CI build published.)* Per-version detail in [`CHANGELOG.md`](CHANGELOG.md).

*(Superseded — kept for context)* v2.3.6 — **a workaround for a Broadcom bug that can permanently freeze saving settings.** A router can reach a state where saving a setting simply never completes. The symptom is unhelpfully quiet: the router stays up, SSH and the web interface still respond, traffic still flows — but one process is stuck forever, a CPU core is consumed doing nothing useful, logs quietly fill, and the setting is never actually written. It becomes more likely the longer a router has been running. The cause is in a **closed Broadcom library** (`libnvram.so`), found by catching it live on hardware and then confirming it instruction-by-instruction in the shipped binary: the library identifies its kernel channel by the process's own ID, and it also keeps a record of *"which process am I"* — and the defect is that the number in use and that identity record are **the same piece of memory**. Whenever the two disagree the library decides the process must have changed and starts over **without closing the channel it already opened**. Two different things make them disagree, and both happen on ordinary routers. First, **on every startup of every router**: most background services read a setting before moving themselves into the background, and doing so gives the running copy a new process ID while it inherits the channel opened under the old one — so the library abandons that first channel, and its number stays occupied while belonging to no running process. Confirmed on a live router: **six abandoned numbers, and four services each holding two channels, twenty-one minutes after a clean boot**, with nothing malfunctioning and nothing yet stuck. Second, those abandoned numbers are what eventually freezes something: process IDs are handed out in sequence and wrap around every 32,768 processes, so sooner or later a new process is handed a number one of them is sitting on. Only then is the conflict real — the library tries the next nine along, and *this* time each attempt overwrites its own identity record. A single collision is survivable on its own: it finds a free number, and the mess disappears when that process exits. The damage needs the affected process to **stay running and keep saving settings**, because its record of its own identity is now permanently wrong — so every later call starts over and abandons another channel, until all ten are gone, after which it retries a failing operation *forever*, with no pause, holding a lock that blocks every further settings operation *within that process*. The rest of the router carries on regardless, which is a large part of why this is so hard to notice. Those abandoned numbers come from healthy services during a normal boot rather than from anything already broken, so every router carries a handful and the ID counter sweeps past each one on every wrap — between one and two hours on a live RT-BE96U. A sweep by itself is harmless, though: measured on that hardware, tens of thousands of short-lived processes crossed all six abandoned numbers without causing a single freeze. What it takes is one of the *long-running* services to restart onto one of those numbers and then go on saving settings — uncommon, unpredictable, and likelier the longer a router runs, which is exactly the shape of a complaint that has followed this platform for years without ever being pinned down. The freeze depends entirely on an attempt failing with "address in use", so Reaper now ships a small piece of its own code that sits beside the Broadcom library — it does **not** modify it — and, only after such a failure, asks the kernel to pick a free number instead. **This prevents the freeze, not the abandonment:** the startup case never fails with "address in use", so those channels are still abandoned exactly as before whether the workaround is on or off; what changes is the outcome when something later lands on one — a rescued conflict and a line in the log, instead of a process stuck for good. Fixing the leak itself is Broadcom's to do. **It ships switched off:** the mechanism applies to every program on the router and has not yet run on real hardware, so nothing changes until you deliberately enable it with `nvram set wlcsm_bindfix=1`, commit and reboot; setting it back to `0` removes it on the next boot. With it on, each rescue writes a line to the system log naming the process and number, so you can tell whether your router is affected at all — read it on the **System Log** page or as the file `/tmp/syslog.log`, not via `logread`, which returns nothing on this platform. **Validation status: the defect is now confirmed on hardware; the workaround is source only — not built, and not run on hardware, on any model.** Per-version detail in [`CHANGELOG.md`](CHANGELOG.md).

*(Superseded — kept for context)* v2.3.5 — **counters that survive a reboot, UPnP that keeps working, and a first-boot setup you cannot walk past.** Five field reports, all of the same shape: a feature that reported success while doing nothing. **Warden's blocked-hit counts** reset on every reboot and firmware upgrade, and the per-country list never accumulated at all — the totals were only saved by a script that runs *after* the firewall rules are torn down, and the per-country table had no saved copy whatsoever, being read straight from live firewall rules that are deleted and rebuilt whenever anything touches the firewall. That is why the downloaded blocklists survived a reboot while the numbers beside them did not. Both are now banked to flash from every path that would lose them, including a 15-minute checkpoint, and you can choose how long they are kept (Never / 24 Hours / 7 Days / 1 Month / 3 Months). **UPnP port mappings** stopped working after a while and nothing said so: any firewall rebuild wipes every mapping, and the router's recovery signal only announced a public-IP change — it re-added nothing, while the service kept telling programs "that port is already forwarded". Consoles losing their NAT type hours after boot, game hosting failing, downloads with no incoming connections: one bug, now fixed by genuinely restarting the service so every mapping is reloaded with its remaining lease. UPnP also now presents itself as an **IGD:2 gateway by default** (previously only if you enabled IPv6 pinholes, which most people sensibly leave off — silently costing IPv4 clients the ability to say "give me any free port if this one is taken"), and it **refuses mappings on the router's own** OpenVPN, WireGuard, IPSec, SSH and web-admin ports, closing a path by which a device on your network could quietly take over inbound VPN connections. The **DHCP Leases** page showed a dash instead of the device name for devices the Devices page names perfectly well — it read from the live network-map rather than from where names are actually stored — and a device name or self-reported hostname containing a quote could blank the whole table; both are fixed, which additionally closes a way for a LAN device to run script in an admin's browser by choosing a crafted hostname. **Smart Connect Rules** now explains its "- -" columns instead of looking broken *(investigating that report also confirmed the page's band mapping is correct — the reporting router simply had 2.4 GHz deliberately excluded)*. And **first-boot setup can no longer be skipped, and now actually asks for the Wi-Fi password**: previously the wizard sent you to the dashboard after setting admin credentials, and the dashboard is one of the pages that does not run the Wi-Fi check — so a router could finish setup with new admin credentials and the factory Wi-Fi password still in place — while the wizard itself could be bypassed entirely by typing almost any settings-page address directly. Enforcement now lives in the web server, so every page is covered, and the wizard will not release you until both the administrator credentials and the Wi-Fi password have been changed. **Validation status: source only — this release has not been built or run on hardware on any model.** The rung is ported to all five model branches; on-device validation, especially of the first-boot gate under a factory reset, is outstanding. Per-version detail in [`CHANGELOG.md`](CHANGELOG.md).

*(Superseded — kept for context)* v2.3.4 — **the Traffic Manager and Traffic Analyzer pages fit the screen again.** Both pages rendered oversized and ran off the right edge with no scrollbar to reach the rest. Each carries a "FAQ" panel that slides in from the right and, while closed, is parked just off the edge of the page; v2.3.3's fix for centring "please wait" overlays on your screen swept that parked panel into the same rule and changed how it is positioned, which made its off-screen parking count as real page width. The page reported itself ~460 pixels wider than it was, the frame stretched to match, and every panel stretched with it — putting the last column outside the window. Only those two pages have that panel. The panel is excluded from the rule and behaves as before; separately, the content area can now scroll, so a case like this pans on a scrollbar instead of hiding content. Nothing else changed. Per-version detail in [`CHANGELOG.md`](CHANGELOG.md).

*(Superseded — kept for context)* v2.3.3 — **the browser stops talking to ASUS entirely, a client-list crash fixed, and two injection paths closed.** The de-cloud is now **finished for the shipping web interface**: earlier releases closed the device/app **icon** callbacks (v2.2.0, v2.2.5); this one closes the remaining **data** surface — FAQ index, DNS-provider list, SDN scenario list, model-name table, AiMesh node-icon table, timezone table, ISP list, IPTV profiles, the Open NAT game database, the VPN partner server lists, and 27 hardcoded game-artwork URLs — each replaced by the copy already bundled in the firmware. The worst offender retried ASUS **every five seconds indefinitely** when the fetch failed. Verified against the **staged filesystem**, not the source tree (several apparent call sites are already inert; one that looked dead was live). Also: a script error that could leave the **client list half-built** when a request timed out is fixed; the **Data Export** destination URL and token are validated at save time, closing a path where a quote could inject directives into the download tool's config file and write a file anywhere as **root**; the AI Advisor's **lab** build no longer returns the Wi-Fi passphrase (its redaction filter didn't recognise ASUS's `>`-separated credential record); hardware QoS stops installing a download-side redirect only the software engine can use; and ~390 lines of proven-unreachable code were removed. A second batch cut into the same release adds the **explanation for the post-upgrade frozen browser** — the page's status polling scheduled *two* successors per round, because a failed request fires both a state-change and an error event, so during the reboot the chain doubled every round until the tab locked up (v2.3.2's Close button treated the symptom; this is the cause) — plus a **stored cross-site-scripting hole** reachable by any device on the network, since a device's DHCP-announced name was pasted into an HTML attribute that is itself JavaScript, where HTML-encoding is the wrong escape. Hardware QoS ships a **measured classful profile** instead of an even five-way split, and its preset rules stop classifying by **bytes transferred**, which had been demoting long video calls and game sessions into the bulk queue simply for lasting a while. The **Traffic Analyzer** was reading its byte counts from the accelerator's flow table, so the fastest downloads — the ones most likely to be offloaded — were the ones most likely to be missing, and guest / SDN clients were dropped entirely. "Please wait" overlays and countdowns now centre on **your screen** rather than inside the framed page; the **Network Map** is retired with the dashboard taking its place in the navigation; and the Firmware, Data Export and USB/storage strings are translated across all **24** non-English languages. Per-version detail in [`CHANGELOG.md`](CHANGELOG.md).

*(Superseded — kept for context)* v2.3.2 — **QoS finally classifies the way its labels say, a firmware-flash screen you can always escape, and a small security sweep.** The classful QoS engine's shipped defaults were working against their own labels — unmatched traffic (games included) landed in the same queue as bulk file transfers, the **DSCP-to-class map was inverted** so a correctly-marked game could never reach the queue named for it, committed minimum rates consumed **100% of the port** leaving the scheduler no slack, and QoS with empty bandwidth fields armed classification and AQM with no shaper at all. All corrected, with the downstream DSCP→WMM stamp made **opt-in** (defaulting it on promoted ordinary browsing to the Voice class on the air). The firmware page's flashing overlay gained a **Close button and Esc** so a stalled flash can no longer look like a frozen browser, plus a live elapsed-time heartbeat; the manual-upload flow no longer declares a good image rejected while the router is still verifying it. Four smaller confirmed items cleared: the QoS diagnostic read the wrong shaper for the running engine, a VPN-provider setup path let an apostrophe escape shell quoting, the password check was made constant-time, and the health watchdog now rate-limits incident dumps to flash. **First release built and published through the public clean-room CI pipeline.** Per-version detail in [`CHANGELOG.md`](CHANGELOG.md).

*(Superseded — kept for context)* v2.3.1 — **a native Firmware page: working update check, one-click verified install, manual upload.** Administration → Firmware Upgrade is now a Reaper-native page: installed model / variant / version at a glance, an on-demand **Check** against the Reaper release channel, **inline release notes**, an opt-in **scheduled daily check** (off by default), **manual upload** with a real progress bar, and — new capability the stock page never offered — **one-click download + install** of a published update with an end-to-end verification chain (Reaper-GitHub-only download host, exact model + variant guard so a noMCP router refuses an MCP image, manifest **SHA-256 + size** match, the platform's own image check) before anything is flashed. Also fixes why the update system looked dead: on AiMesh-config-sync builds the stock Check silently asked **ASUS's servers** (a closed helper), so the v2.1.6 Reaper release-channel check never ran from the GUI and the dashboard badge never fired; the release-note fetcher also wrote where no page read. Fixed on the native page and the stock fallback alike. Nothing phones home by default. Per-version detail in [`CHANGELOG.md`](CHANGELOG.md).

*(Superseded — kept for context)* v2.3.0 — **animated model header, a standalone health-probe switch, a real "Store only" export mode, and one row per device in the metrics.** The **sign-in / set-password / logout** screens gain an **animated model header** that plays once and freezes on the logo (an APNG served through the normal image path, so it needs no change to how the login page is served). The per-device **health probe** (latency / jitter / loss + TCP state) gets a **standalone on/off switch** on Administration -> Data Export, **independent of export**, so you can collect and **Preview** the metrics with no external destination (Preview now shows a helpful message instead of empty ``{}``). The Long-Term Storage export control now has **four modes -- Off / Store only / Store + Export / Export only** -- with the misleading "Off" label fixed. And a device that could show **twice** (once by MAC, once with its IP in the MAC field) is folded to **one row per device**. Also: Warden's total-blocked count truly persists across reboots; the apply/reboot overlay locks the header + nav during a framed load; the update-check log lines are de-Merlin'd; the Traffic Analyzer hides its own "Router" self-traffic row; and the Roaming Assistant help reads just "0 = off". Rides the v2.2 line (connection-health metrics + analytics export, the speed-test freeze fix, the first-boot loop fix, Gatekeeper-behind-LAN-DNS, the de-cloud/field-fix batch). A four-agent pre-distribution review found **no shipping blockers**; its follow-ups were folded in (the Health-probe toggle now refuses to turn off while an export mode is armed -- which would otherwise freeze the pushed feed; the Storage export note flags that non-off modes enable the probe; a dead leftover `.mp4` httpd handler removed). **Built + shipped on all five models, both variants each.** Per-version detail in [`CHANGELOG.md`](CHANGELOG.md).
**Firmware:** `3006.102.8_Reaper_v2.4.3` — current head; the rung is cut (428 patches, replay-verified) and built on RT-BE96U in both variants (20 pass / 0 warn / 0 FAIL each), but **no image has been published from it**. It touches no per-model overlay, so the four siblings take it from the patch series unchanged. v2.4.2 (426 patches, built on RT-BE96U MCP and run on hardware) published no image either and is folded into it. **Newest PUBLISHED release: v2.3.7**, on all five models — that is the newest image a user can actually install, and it is what "current version" means in [`../README.md`](../README.md). v2.3.8, v2.3.9 and v2.4.0 are **source rungs that published no images**; v2.4.1 is built on RT-BE96U (both variants) and run on hardware, with its sibling fan-out owed. Prior full-fleet published releases: **v2.3.4**, **v2.3.2**, **v2.3.1**, **v2.3.0**.
**Base:** Asuswrt-Merlin 3006.102.8 (upstream RMerl/asuswrt-merlin.ng)
**Models:** ASUS **RT-BEXXU** (primary), plus the **RT-BE86U**, **RT-BE88U**, **GT-BE98**, and **GT-BE98 Pro** siblings (per-model branches of the same tree). **v2.3.7 is published on all five models** (both variants each). WiFi 7, Broadcom BCM4916.
...
> Naming note: the build artifacts are `…_Reaper_v2.3.7_nand_squashfs.pkgtb`
> (**with** the Advisor) and `…_Reaper_v2.3.7_noMCP_…` (**without**). §8 lists the
> exact filenames and their hashes per release. **v2.3.2** is the current full-fleet
> set (all five models, both variants, published through the CI pipeline);
> **v2.3.4** is the current source rung (RT-BE96U-only so far, both variants); it is a single-defect
> fix on top of v2.3.3, and sibling fan-out plus on-device validation are pending.
...
through the current **v2.3.1** — is built + shipped. **v2.0.0** had its **upgrade-path first boot verified on the physical
...
rungs were RT-BE96U-only; **v2.2.5, v2.2.6, v2.3.0, and v2.3.1 are each
built + shipped on all five models, both variants** — every build as itself, per-model
banner/base/BUILD_NAME verified by the 19-check staged-image gate and the noMCP images
confirmed Advisor-free. Since then: **v2.3.2** and **v2.3.4** shipped on all five models,
**v2.3.7** is the newest release published on all five, and **v2.4.1** is the newest built
version — RT-BE96U only, both variants, run on hardware, with its sibling fan-out owed.
*(This paragraph previously described v2.3.1 as "current head", which it stopped being on
2026-08-08; the head is now stated in one place, in the Firmware line at the top of this file.)* **On-hardware validation of the entire v2.1.x + v2.2.x line is owed** — in particular, enabling the
