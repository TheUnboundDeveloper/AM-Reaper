...
**Firmware:** `3006.102.8_Reaper_v2.3.5` -- current head, **source rung only: not built, and not run on hardware, on any model.** The rung is ported to all five model branches (RT-BE96U canon + the four siblings, each guard-checked with zero unsynced shared files), so a build can be cut for any of them; nothing here has produced an image yet. Last built release: **v2.3.4** (RT-BE96U, both variants). Prior full-fleet releases: **v2.3.2**, **v2.3.0**, **v2.2.6**.
**Base:** Asuswrt-Merlin 3006.102.8 (upstream RMerl/asuswrt-merlin.ng)
**Models:** ASUS **RT-BEXXU** (primary), plus the **RT-BE86U**, **RT-BE88U**, **GT-BE98**, and **GT-BE98 Pro** siblings (per-model branches of the same tree). **v2.3.2 is built + shipped on all five models** (both variants each). WiFi 7, Broadcom BCM4916.
...
|---|---|---|
| **Standard** | `RT-BEXXU_…_Reaper_v2.3.3_noMCP_nand_squashfs.pkgtb` | **No — never compiled in** |
| **+ AI Advisor** | `RT-BEXXU_…_Reaper_v2.3.3_nand_squashfs.pkgtb` | Yes (optional, off by default) |
...
> Naming note: the build artifacts are `…_Reaper_v2.3.3_nand_squashfs.pkgtb`
> (**with** the Advisor) and `…_Reaper_v2.3.3_noMCP_…` (**without**). §8 lists the
...
**Firmware:** `3006.102.8_Reaper_v2.3.1` -- current head, built + shipped on all five models (both variants each).
...
rungs were RT-BE96U-only; **v2.2.5, v2.2.6, v2.3.0, and v2.3.1 (current head, 2026-08-08) are each
