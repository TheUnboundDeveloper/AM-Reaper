...
**Firmware:** `3006.102.8_Reaper_v2.4.2` — current head; the rung is cut (426 patches, replay-verified), built on RT-BE96U MCP and **run on hardware**, with the four sibling branches ported and their overlays regenerated, but **no image has been published from it**. **Newest PUBLISHED release: v2.3.7**, on all five models — that is the newest image a user can actually install, and it is what "current version" means in [`../README.md`](../README.md). v2.3.8, v2.3.9 and v2.4.0 are **source rungs that published no images**; v2.4.1 is built on RT-BE96U (both variants) and run on hardware, with its sibling fan-out owed. Prior full-fleet published releases: **v2.3.4**, **v2.3.2**, **v2.3.1**, **v2.3.0**.
