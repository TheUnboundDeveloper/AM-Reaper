...
**Release:** v2.3.6 — **a workaround for a Broadcom bug that can permanently freeze saving settings.** A router can reach a state where saving a setting simply never completes. The symptom is unhelpfully quiet: the router stays up, SSH and the web interface still respond, traffic still flows — but one process is stuck forever, a CPU core is consumed doing nothing useful, logs quietly fill, and the setting is never actually written. It becomes more likely the longer a router has been running. The cause is in a **closed Broadcom library** (`libnvram.so`), found by catching it live on hardware and then confirming it instruction-by-instruction in the shipped binary: the library identifies its kernel channel by the process's own ID, and when that number is already taken — which happens naturally as process IDs get reused — it tries the next nine along. The defect is that the number being tried and the library's record of *"which process am I"* are **the same piece of memory**, so the moment it has to try a different number it corrupts its own identity record. The next call sees the mismatch, wrongly decides the process changed, and starts over **without closing the channel it already opened** — leaking one per round until all ten numbers are gone, after which it retries a failing operation *forever*, with no pause, while holding the lock every other settings operation needs. The leaked channels stay held for the life of the stuck process, which never exits, so they make the next collision more likely: it compounds with uptime. Every step of that chain depends on the *first* attempt failing, so Reaper now ships a small piece of its own code that sits beside the Broadcom library — it does **not** modify it — and, only when that first attempt has already failed with "address in use", asks the kernel to pick a free number instead. That is precisely what Broadcom's own second channel already does. **It ships switched off:** the mechanism applies to every program on the router and has not yet run on real hardware, so nothing changes until you deliberately enable it with `nvram set wlcsm_bindfix=1`, commit and reboot; setting it back to `0` removes it on the next boot. With it on, each rescue writes a line to the system log naming the process and number, so you can tell whether your router is affected at all. **Validation status: source only — not built, and not run on hardware, on any model.** Per-version detail in [`CHANGELOG.md`](CHANGELOG.md).
