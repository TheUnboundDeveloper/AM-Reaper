...
**Firmware:** `3006.102.8_Reaper_v2.4.1` — newest **built** version, and the version string the tree still carries. A further rung of work sits **uncommitted and unbuilt** on top of it (see "In development" above); it has no version number yet, so nothing in the tree claims to contain it. **Newest PUBLISHED release: v2.3.7**, on all five models — that is the newest image a user can actually install, and it is what "current version" means in [`../README.md`](../README.md). v2.3.8, v2.3.9 and v2.4.0 are **source rungs that published no images**; v2.4.1 is built on RT-BE96U (both variants) and its metal validation and sibling fan-out are owed. Prior full-fleet published releases: **v2.3.4**, **v2.3.2**, **v2.3.1**, **v2.3.0**.
